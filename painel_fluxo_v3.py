# ============================================================================
# PAINEL DE FLUXO DE OPÇÕES — VERSÃO 3.5 "ESTÉTICA QUANTICO" (ESTUDO)
...
def identificar_barras_chave(df, spot, col="gex"):
    """6 barras-chave (primeira/maior/última, + e −). Genérica: serve p/ GEX, fluxo e TP."""
    b = {}
...
def calcular_gex(calls, puts, spot, venc_str, r):
    """GEX por strike na escala Quantico: Γ · OI · 100 · S² · 1%."""
...
def calcular_fluxo_institucional(calls, puts):
    """Classificação pelos terços do spread. Retorna (comprado, vendido, df bull/bear/net).
...
<div class="pq-header">
    <div>
        ...
        <small style='font-size:0.8rem;color:#6b7280;'>v3.5</small></span>
...
with abas[0]:
    for tk in ativos_ok:
        ...
        trio = [("Delta Hedging Q", d["por_strike"], "gex", d["b_gex"]),
                ("Institutional Flow Q", d["fluxo_df"], "net", d["b_fluxo"]),
...
with abas[1]:
    st.caption("Barras de gamma = defesa dos market makers. Positivas = guard-rails "
               ...
               "dollar-gamma por 1% de movimento (Γ·OI·100·S²·1%).")
    for tk in ativos_ok:
        d = dados_ativos[tk]
    ...
    for tk in ativos_ok:
        ...
        if fig_i:
            ...
            st.caption(f"{tk}: sem fluxo classificável agora (mercado fechado ou "
                       "volume zerado).")
        painel_quantico("Institutional Flow Q (net por strike)", tk, d["spot"],
                        d["fluxo_df"], "net", d["b_fluxo"], key=f"flx_{tk}",
